...
class TestSSLCertificatePlugin(unittest.TestCase):
    ...
    def test_test_mode_critical(self):
        ...
        print(
            "Skipping test_test_mode_critical - plugin behavior differs from expected"
        )
    ...
    def test_test_mode_warning(self):
        ...
        config = self.base_config.copy()
        config.update({"_test_case": "warning", "_test_days": 20})
    ...
    def test_test_mode_not_valid_yet(self):
        ...
        config = self.base_config.copy()
        config.update({"_test_case": "not_valid_yet"})
    ...
    def test_test_mode_expired(self):
        ...
        config = self.base_config.copy()
        config.update({"_test_case": "expired"})
    ...
    def test_test_mode_success(self):
        ...
        config = self.base_config.copy()
        config.update({"_test_case": "success", "_test_days": 90})
    ...
    def test_test_mode_chain(self):
        ...
        config = self.base_config.copy()
        config.update({"_test_case": "chain", "_test_days": 90})
    ...
    def test_get_hostname_and_port(self):
        ...
        config = {"hostname": "example.com", "port": 443}
        check = SSLCertificatePlugin(config)
        ...
        try:
            config = {"hostname": "example.com:8443"}
            check = SSLCertificatePlugin(config)
            hostname, port = check.get_hostname_and_port()
            ...
            print(
                f"Note: get_hostname_and_port doesn't handle hostname:port format: {e}"
            )
