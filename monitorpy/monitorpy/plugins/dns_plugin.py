...
class DNSRecordPlugin(MonitorPlugin):
    ...
    def validate_config(self) -> bool:
        ...
        if not DNSPYTHON_AVAILABLE:
            logger.error("dnspython library is not installed. Please install it using 'pip install dnspython'")
            return False
        ...
        if record_type not in valid_types:
            logger.error(f"Invalid record type: {record_type}. Must be one of: {', '.join(valid_types)}")
            return False

        # If expected_value is required for certain record types
        if record_type in ["A", "AAAA", "CNAME", "TXT"] and "expected_value" not in self.config:
            # It's not a hard requirement, but log a warning
            logger.warning(f"No expected_value provided for {record_type} record. Will only check existence.")
        ...
        if self.config.get("check_propagation", False):
            ...
            if not isinstance(threshold, (int, float)) or threshold < 0 or threshold > 100:
                logger.error("Propagation threshold must be a percentage between 0 and 100")
                return False
        ...
        if "resolvers" in self.config:
            ...
            if not isinstance(resolvers, list):
                logger.error("Resolvers must be a list of IP addresses or resolver objects")
                return False
    ...
    def run_check(self) -> CheckResult:
        ...
        try:
            ...
            if nameserver:
                resolver.nameservers = [nameserver] if isinstance(nameserver, str) else nameserver
            ...
            try:
                ...
                logger.debug(f"Found {len(answer_values)} {record_type} records for {full_domain}")
                ...
                if check_authoritative:
                    authoritative_result = self._check_authoritative(full_domain, record_type)
                ...
                if expected_value:
                    ...
                    if isinstance(expected_value, list):
                        ...
                        expected_value_match = all(val in answer_values for val in expected_value)
                    else:
                ...
                if expected_value and not expected_value_match:
                    ...
                    if isinstance(expected_value, list):
                        message_parts.append(f"Expected values {expected_value} not all found")
                    else:
                        message_parts.append(f"Expected value '{expected_value}' not found")

                # Check authoritative result if present
                if authoritative_result and not authoritative_result.get("is_authoritative", True):
                ...
                if propagation_result:
                    ...
                    if prop_status == CheckResult.STATUS_ERROR:
                        ...
                        if status != CheckResult.STATUS_ERROR:  # Don't downgrade from ERROR
                            status = CheckResult.STATUS_WARNING
                        message_parts.append(
                ...
                if status == CheckResult.STATUS_SUCCESS:
                    if expected_value:
                        ...
                        if len(answer_values) > 1:
                            message = f"DNS {record_type} records found for {full_domain}"
                else:
                    if message_parts:
                        message = f"DNS {record_type} record check for {full_domain} has issues: {', '.join(message_parts)}"
                    else:
                        message = f"DNS {record_type} record check for {full_domain} failed"
    ...
    def _format_answers(self, answers: dns.resolver.Answer, record_type: str) -> List[str]:
        """
        ...
        for rdata in answers:
            if record_type == "A" or record_type == "AAAA":
                ...
                # SRV records
                result.append(f"{rdata.priority} {rdata.weight} {rdata.port} {rdata.target}")
        ...
        try:
            ...
            for ns in auth_ns:
                try:
                    ...
                    logger.debug(f"Error querying authoritative nameserver {ns}: {str(e)}")
                    continue
    ...
    def _check_dnssec(self, domain: str, record_type: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "is_valid": False,
                "error": f"Error checking DNSSEC: {str(e)}"
            }
    ...
    def _check_propagation(
        ...
        if isinstance(resolvers, list) and all(isinstance(r, str) for r in resolvers):
            ...
            resolvers = [{"ip": ip, "name": ip, "provider": "Custom"} for ip in resolvers]
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for future in as_completed(future_to_resolver):
                ...
                try:
                    ...
                    logger.debug(f"Error processing resolver {resolver['ip']}: {str(e)}")
                    results.append({
        ...
        percentage = (consistent_resolvers / total_resolvers * 100) if total_resolvers > 0 else 0

        # Determine status based on threshold
        if percentage >= propagation_threshold:
            status = CheckResult.STATUS_SUCCESS
        elif percentage >= propagation_threshold * 0.7:  # 70% of threshold as warning level
            status = CheckResult.STATUS_WARNING
        else:
            status = CheckResult.STATUS_ERROR
